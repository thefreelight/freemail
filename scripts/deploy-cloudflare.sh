...
MAIL_DOMAIN="${MAIL_DOMAIN:-freemail.123kele.com,yelu.edu.kg}"
...
SENDFLARE_API_KEY="${SENDFLARE_API_KEY:-}" \
CYBERPERSONS_API_KEY="${CYBERPERSONS_API_KEY:-}" \
SMTP_HOST="${SMTP_HOST:-}" \
SMTP_PORT="${SMTP_PORT:-}" \
SMTP_USER="${SMTP_USER:-}" \
SMTP_FROM="${SMTP_FROM:-}" \
...
replaceAssignment('SENDFLARE_API_KEY', process.env.SENDFLARE_API_KEY);
replaceAssignment('CYBERPERSONS_API_KEY', process.env.CYBERPERSONS_API_KEY);
for (const key of ['SMTP_HOST', 'SMTP_PORT', 'SMTP_USER', 'SMTP_FROM', 'SMTP_HELO', 'SMTP_ENVELOPE_FROM']) {
  if (process.env[key]) replaceAssignment(key, process.env[key]);
}
...
if ! "$WRANGLER" r2 bucket list | grep -Eq "^name:[[:space:]]+${R2_BUCKET_NAME}$"; then
