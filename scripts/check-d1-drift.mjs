...
function parseExpectedSchema(sql) {
  ...
  for (const match of sql.matchAll(indexPattern)) {
    indexes.set(match[1], { table: match[2], columns: normalizeIndexColumns(match[3]) });
  }
...
async function readRemoteSchema(databaseName, configPath) {
  ...
  for (const row of rows) {
    if (row.type === 'table') {
      ...
      tables.set(row.name, columns.map((column) => column.name));
    }
    if (row.type === 'index') {
      const columns = await queryD1(databaseName, configPath, `PRAGMA index_info(${quoteIdentifier(row.name)})`);
      indexes.set(row.name, { table: row.tbl_name, columns: columns.map((column) => column.name) });
    }
  }
  return { tables, indexes };
}

function queryD1(databaseName, configPath, command) {
...
function compareSchemas(expectedSchema, remoteSchema) {
  ...
  for (const [table, expectedColumns] of expectedSchema.tables) {
    ...
    for (const column of expectedColumns) if (!remoteSet.has(column)) problems.push(`missing column: ${table}.${column}`);
    for (const column of remoteColumns) if (!expectedSet.has(column)) problems.push(`unexpected column: ${table}.${column}`);
  }
  for (const table of remoteSchema.tables.keys()) if (!expectedSchema.tables.has(table)) problems.push(`unexpected table: ${table}`);
  for (const [index, expectedIndex] of expectedSchema.indexes) {
    ...
    if (remoteIndex.table !== expectedIndex.table) problems.push(`index table mismatch: ${index} expected ${expectedIndex.table}, got ${remoteIndex.table}`);
    const expectedColumns = expectedIndex.columns.join(',');
    const remoteColumns = remoteIndex.columns.join(',');
    if (remoteColumns !== expectedColumns) problems.push(`index columns mismatch: ${index} expected (${expectedColumns}), got (${remoteColumns})`);
  }
  for (const index of remoteSchema.indexes.keys()) if (!expectedSchema.indexes.has(index)) problems.push(`unexpected index: ${index}`);
  return problems;
}

function normalizeIndexColumns(value) {
  return value.split(',').map((column) => column.trim().replace(/\s+DESC$/i, '').replace(/\s+ASC$/i, '')).filter(Boolean);
}
